...
output "cloud_monitoring_access_key" {
  value       = length(ibm_resource_key.cloud_monitoring_resource_key) > 0 ? ibm_resource_key.cloud_monitoring_resource_key[0].credentials["Sysdig Access Key"] : null
  description = "IBM cloud monitoring access key for agents to use"
  sensitive   = true
}

output "cloud_monitoring_manager_key_name" {
  value       = length(ibm_resource_key.cloud_monitoring_resource_key) > 0 ? ibm_resource_key.cloud_monitoring_resource_key[0].name : null
