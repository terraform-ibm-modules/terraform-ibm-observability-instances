...
variable "logs_routing_tenant_regions" {
  type        = list(any)
  default     = []
  description = "Pass a list of regions to create a tenant for that is targetted to the Cloud Logs instance created by this module. To manage platform logs that are generated by IBM Cloud® services in a region of IBM Cloud, you must create a tenant in each region that you operate. Leave the list empty if you don't want to create any tenants. CURRENT LIMITATION: This list can currently only contain 1 region, and it must be the same region that is defined in the provider config used when consuming this module."
  nullable    = false

  validation {
    condition     = (length(var.logs_routing_tenant_regions) == 0 || length(var.logs_routing_tenant_regions) == 1)
    error_message = "CURRENT LIMITATION: This list can currently only contain 1 region, and it must be the same region that is defined in the provider config used when consuming this module. Leave the list empty if you don't want to create any tenants."
  }
}
