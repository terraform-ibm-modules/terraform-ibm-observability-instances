...
| <a name="input_cbr_rules_sysdig"></a> [cbr\_rules\_sysdig](#input\_cbr\_rules\_sysdig) | (Optional, list) List of context-based restrictions rules to create for IBM cloud monitoring instance. | <pre>list(object({<br/>    description = string<br/>    account_id  = string<br/>    rule_contexts = list(object({<br/>      attributes = optional(list(object({<br/>        name  = string<br/>        value = string<br/>    }))) }))<br/>    enforcement_mode = string<br/>    operations = optional(list(object({<br/>      api_types = list(object({<br/>        api_type_id = string<br/>      }))<br/>    })))<br/>  }))</pre> | `[]` | no |
