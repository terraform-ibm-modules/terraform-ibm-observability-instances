...
| <a name="input_metrics_router_routes"></a> [metrics\_router\_routes](#input\_metrics\_router\_routes) | List of routes for IBM Metrics Router. | <pre>list(object({<br>    name = string<br>    rules = list(object({<br>      action = optional(string, "send")<br>      targets = list(object({<br>        id = optional(string)<br>      }))<br>      inclusion_filters = list(object({<br>        operand  = string<br>        operator = string<br>        values   = list(string)<br>      }))<br>    }))<br>  }))</pre> | `[]` | no |
