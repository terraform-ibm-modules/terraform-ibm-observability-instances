...
- [ Provision Activity Tracker along with event routing to COS bucket, Event streams and LogDNA](examples/observability_at_event_routing)
...
| <a name="input_activity_tracker_routes"></a> [activity\_tracker\_routes](#input\_activity\_tracker\_routes) | Map of routes to be created, maximum four routes are allowed | <pre>map(object({<br>    locations  = list(string)<br>    target_ids = list(string)<br>  }))</pre> | `{}` | no |
...
| <a name="input_cos_targets"></a> [cos\_targets](#input\_cos\_targets) | cos\_target = {<br>      cos\_endpoint: "(Object) Property values for COS Endpoint"<br>      target\_name: "(String) The name of the COS target."<br>      target\_region: "(String) Region where is COS target is created, include this field if you want to create a target in a different region other than the one you are connected"<br>    } | <pre>map(object({<br>    cos_endpoint = object({<br>      endpoint                   = string<br>      bucket_name                = string<br>      target_crn                 = string<br>      api_key                    = optional(string)<br>      service_to_service_enabled = optional(bool, false)<br>    })<br>    target_name   = string<br>    target_region = optional(string)<br>  }))</pre> | `{}` | no |
| <a name="input_default_targets"></a> [default\_targets](#input\_default\_targets) | The target ID List. In the event that no routing rule causes the event to be sent to a target, these targets will receive the event | `list(string)` | `[]` | no |
...
| <a name="input_eventstreams_targets"></a> [eventstreams\_targets](#input\_eventstreams\_targets) | eventstreams\_target = {<br>      eventstreams\_endpoint: "(Object) Property values for event streams Endpoint"<br>      target\_name: "(String) The name of the event streams target."<br>      target\_region: "(String) Region where is event streams target is created, include this field if you want to create a target in a different region other than the one you are connected"<br>    } | <pre>map(object({<br>    eventstreams_endpoint = object({<br>      target_crn = string<br>      brokers    = list(string)<br>      topic      = string<br>      api_key    = string<br>    })<br>    target_name   = string<br>    target_region = optional(string)<br>  }))</pre> | `{}` | no |
...
| <a name="input_logdna_targets"></a> [logdna\_targets](#input\_logdna\_targets) | logdna\_target = {<br>      logdna\_endpoint: "(Object) Property values for LogDNA Endpoint"<br>      target\_name: "(String) The name of the logDNA target."<br>      target\_region: "(String) Region where is LogDNA target is created, include this field if you want to create a target in a different region other than the one you are connected"<br>    } | <pre>map(object({<br>    logdna_endpoint = object({<br>      target_crn    = string<br>      ingestion_key = string<br>    })<br>    target_name   = string<br>    target_region = optional(string)<br>  }))</pre> | `{}` | no |
| <a name="input_metadata_region_backup"></a> [metadata\_region\_backup](#input\_metadata\_region\_backup) | Backup region to store all your meta data in a . | `string` | `"us-east"` | no |
| <a name="input_metadata_region_primary"></a> [metadata\_region\_primary](#input\_metadata\_region\_primary) | Primary region to store all your meta data. | `string` | `"us-south"` | no |
| <a name="input_permitted_target_regions"></a> [permitted\_target\_regions](#input\_permitted\_target\_regions) | List of regions where target can be defined. | `list(string)` | <pre>[<br>  "us-south",<br>  "eu-de",<br>  "us-east",<br>  "eu-gb",<br>  "au-syd"<br>]</pre> | no |
| <a name="input_private_api_endpoint_only"></a> [private\_api\_endpoint\_only](#input\_private\_api\_endpoint\_only) | Set this true to restrict access only to private api endpoint. | `bool` | `false` | no |
...
| <a name="output_activity_tracker_routes"></a> [activity\_tracker\_routes](#output\_activity\_tracker\_routes) | Map of created routes |
| <a name="output_activity_tracker_targets"></a> [activity\_tracker\_targets](#output\_activity\_tracker\_targets) | Map of created targets |
