...
| <a name="input_logdna_targets"></a> [logdna\_targets](#input\_logdna\_targets) | logdna\_target = {<br>      logdna\_endpoint: "(Object) Property values for LogDNA Endpoint"<br>      target\_region: "(String) Region where is LogDNA target is created, include this field if you want to create a target in a different region other than the one you are connected"<br>    } | <pre>map(object({<br>    logdna_endpoint = object({<br>      target_crn    = string<br>      ingestion_key = string<br>    })<br>    target_region = optional(string)<br>  }))</pre> | `{}` | no |
| <a name="input_metadata_region_backup"></a> [metadata\_region\_backup](#input\_metadata\_region\_backup) | Backup region to store all your meta data in a . | `string` | `null` | no |
| <a name="input_metadata_region_primary"></a> [metadata\_region\_primary](#input\_metadata\_region\_primary) | Primary region to store all your meta data. | `string` | `null` | no |
| <a name="input_permitted_target_regions"></a> [permitted\_target\_regions](#input\_permitted\_target\_regions) | List of regions where target can be defined. | `list(string)` | `[]` | no |
